...
const DATA = {
  ...
  weapons: {
    ...
    blizzard: {
      name: 'ブリザード', desc: '吹雪で敵を切り刻み移動速度を低下させる', icon: 'blizzard',
      lv: [
        { cd: 4.5, dmg: 5,  radius: 70,  dur: 2.5, slow: 0.55 },
        { cd: 4.2, dmg: 7,  radius: 80,  dur: 3.0, slow: 0.5 },
        { cd: 3.8, dmg: 9,  radius: 90,  dur: 3.0, slow: 0.5 },
        { cd: 3.4, dmg: 12, radius: 105, dur: 3.2, slow: 0.45 },
        { cd: 3.0, dmg: 16, radius: 120, dur: 3.5, slow: 0.4 },
      ],
    },
  },
  ...
  artifacts: {
    ...
    chain:     { name: '連鎖の雷核',       desc: 'サンダーの落雷ダメージ -35%。落雷が直撃地点から最も近い敵 1 体に連鎖する' },
    frenzy:    { name: '狂戦士の血晶',     desc: 'パワークリスタルの効力が現在HPに比例して変化する(HP満タンで150%・瀕死で0%)' },
    glass:     { name: 'ガラスの大剣',     desc: '攻撃力 +20%。しかし最大HP -30%' },
    clock:     { name: '狂気の懐中時計',   desc: 'クールダウン -10%。しかし攻撃力 -10%' },
    fireburst: { name: '爆裂の火薬庫',     desc: 'ファイアーが着弾時に爆発し範囲ダメージ(威力60%)を与える。射程 -25%' },
    bleed:     { name: '渇血の棘',         desc: 'オービットブレードとスローイングアックスが出血を付与。出血した敵は受けるダメージ +15%' },
    holyleech: { name: '聖杯の加護',       desc: 'ホーリーオーラがダメージを与えるたびHPを 1% 回復する' },
    wisphunter:{ name: '狩猟の精霊石',     desc: 'スピリットウィスプの射程・追尾能力・弾速 +20%。ダメージ -20%' },
    blizzwalk: { name: '吹雪の羅針盤',     desc: 'ブリザードがプレイヤーへ徐々に移動するようになる。範囲 -10%' },
    mirror:    { name: '双面の魔鏡',       desc: 'マジックボルトが反対方向にも同時発射される。ダメージ -25%' },
    critdmg:   { name: '処刑人の刻印',     desc: 'クリティカルダメージ +25%' },
    pact:      { name: '悪魔の契約書',     desc: '獲得経験値 +30%。しかし敵のステータス(HP・速度・攻撃力) +15%' },
  },
